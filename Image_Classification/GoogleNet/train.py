...
def train_model(model, train_loader, test_loader, device):
    ...
    cwd = os.getcwd()
    save_path = cwd + '/Result/googlenet/googlenetmodel_cifar100.pkl'    # 保存模型和模型名字
    # save_path = cwd + '/Result/googlenet/googlenetmodel_flower.pkl'    # 保存模型和模型名字
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in train_bar:
            ...
            outputs, aux_logits2, aux_logits1 = model(images)

            loss0 = loss_function(outputs, labels)
...
if __name__ == "__main__":
    ...
    path0 = os.getcwd()
    save_path = path0 + "/Result/googlenet"
    ...
    # 实例化模型
    net = GoogleNetmodel(num_classes=100, aux_logits=True, init_weights=True)
    net.to(device)
    ...
    # # 实例化模型
    # net = GoogleNetmodel(num_classes=5, aux_logits=True, init_weights=True)
    # net.to(device)
